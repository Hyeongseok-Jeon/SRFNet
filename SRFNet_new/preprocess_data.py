...
#             t = time.time()
#
#     f = open(os.path.join(root_path, 'preprocess_GAN', save), 'wb')
...
for i in range(len(list)):
    ...
    actors_reshape = actors.view(1, actors.shape[1], 32, -1)
    mask = torch.cat((torch.zeros_like(data[0]), torch.zeros_like(data[0])[:, :2, :, :, :, :]), dim=1)
    mask[:, :12] = data[0][:,:12]
    mask[:, 12, :actors.shape[1], :32, :4, 0] = actors_reshape
    mask[:, 13, :, 0, 0, 0] = actors_idcs
    data_mod.append(mask)
