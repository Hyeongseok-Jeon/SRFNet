...
class Loss(nn.Module):
    ...
    def forward(self, target_gt_traj, target_fut_traj, dis_real, dis_fake, hidden_real, hidden_fake, mu_hidden_ego, log_var_hidden_ego, data):
        target_gt_traj_cat = torch.cat([torch.repeat_interleave(target_gt_traj[i].unsqueeze(dim=0), self.config["num_mods"], dim=0) for i in range(len(target_gt_traj))], dim=0)
        target_fut_traj_cat = torch.cat([target_fut_traj['reg'][i].squeeze() for i in range(len(target_gt_traj))])
        l1loss_trajectory = self.reg_loss(target_gt_traj_cat, target_fut_traj_cat) / self.config["num_mods"]
        hidden_real_cat = torch.cat([torch.repeat_interleave(hidden_real[i:i+1,:], self.config["num_mods"], dim=0) for i in range(len(target_gt_traj))], dim=0)
