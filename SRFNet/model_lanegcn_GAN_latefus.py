...
class lanegcn_vanilla_gan(nn.Module):
    ...
    def forward(self, data):
        ...
        mus, log_vars = self.ego_react_encoder(ego_fut_traj, cl_cands_target, init_pred_global)
        ...
        output = init_pred_global
        output['reg'] = [output['reg'][i]+delta[i] for i in range(len(ego_fut_traj))]
...
class EgoReactEncodeNet(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.enc1 = nn.Linear(104, config['n_actor'])
    ...
    def forward(self, ego_fut_traj, cl_cands_target, init_pred_global):
        ...
        data_cat = torch.cat([data[i].view(-1, 104) for i in range(len(data))], dim=0)
    ...
    def reform(self, ego_fut_traj, cl_cands_target, init_pred_global):
        ...
        for i in range(len(cl_cands_target)):
            ...
            for j in range(30):
                ...
                mask = torch.repeat_interleave(mask, 2, dim=0)[:50, 0]
