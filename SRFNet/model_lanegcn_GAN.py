...
class lanegcn_vanilla_gan(nn.Module):
    def __init__(self, config, args):
        ...
        self.config = config
        self.encoder = EncodeNet(config)
        self.ego_encoder = EgoEncodeNet(config)
        self.generator = GenerateNet(config)
        self.discriminator = DicriminateNet(config)
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, data: Dict) -> Dict:
        if 'wrapper_mid_fusion' in self.args.case:
...
def get_model(args):
    if args.case == 'case_1_1':
        ...
        loss = [Loss(config, args).cuda(), L1loss(config).cuda()]
